...
namespace Xamarin_SYM_IOS.SRC.Utils
{
	...
	public class YouTubePlayer
	{
		...
		const string HTMLTemplatePath = @"SRC/Utils/YTPlayerView-iframe-player.html";
		...
		private void SetupPlayerView(CGRect frame)
		{
            ...
            //PlayerView.Frame = new CGRect(0, 0, 300, 300);
		...
		public List<PlaybackQuality> GetAvailableQualityLevels()
		{
			...
			//This call doesn't seem to be returning anything.
		...
		{
			this.LoadPlaylistFromCueStr(string.Format("'{0}'", playlistId),
                                        index, 
                                        startSeconds, 
                                        suggestedQuality);
		}
		...
		{
			EvaluateJavascript(string.Format("player.cuePlaylist({0}, {1}, {2},'{3}');",
                                             cueString, 
                                             index, 
                                             startSeconds,
				                             GetStringFromPlaybackQuality(suggestedQuality)));
